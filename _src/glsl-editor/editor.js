...
const chalky = "var(--chalky)", coral = "var(--coral)", cyan = "var(--cyan)",
      invalid = "var(--invalid)", ivory = "var(--ivory)",
      stone = "var(--stone)", malibu = "var(--malibu)", sage = "var(--sage)",
      whiskey = "var(--whiskey)", violet = "var(--violet)",
      dimColor = "var(--dim-color)",
      highlightBackground = "var(--bg-dim-color)",
    background = "var(--bg-color)", tooltipBackground = "var(--bg-dim-color);",
      selection = "var(--acc-color);", cursor = "var(--fg-color)";
...
const oneDarkTheme = EditorView.theme({
    ...
    ".cm-searchMatch" :
        {backgroundColor : "#72a1ff59", outline : "1px solid #457dff"},
    ...
    ".cm-gutters" :
        {backgroundColor : background, color : stone, border : "none"},

    ".cm-activeLineGutter" : {backgroundColor : highlightBackground},

    ".cm-foldPlaceholder" :
        {backgroundColor : "transparent", border : "none", color : "#ddd"},
    ...
    ".cm-tooltip-autocomplete" : {
        "& > ul > li[aria-selected]" :
            {backgroundColor : highlightBackground, color : ivory}
    }
});
...
function Editor(props) {
    return {
        ...
        mounted() {
            ...
            default: {
                throw new Error("Unhandled editor language", this.lang,
                                codemirror);
            }
            }

            this.view = new EditorView(
                {doc : this.initialText, extensions, parent : codemirror});
...
document.querySelectorAll(".editor").forEach(el => {
    ...
    editors.push({doc : el.textContent.trim(), lang : el.dataset.lang});
