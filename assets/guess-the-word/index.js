function getwords(type) {
    let number = Math.floor(Math.random() * ((type === 'words' ? 60 : 119) + 1));
    let url = `/assets/guess-the-word/${type}/${number}.json`;
    return fetch(url)
        .then(response => {
            if (response.ok) {
...
function fetchDefinition(word) {
    const url = `https://api.wordnik.com/v4/word.json/${word}/definitions?limit=1&includeRelated=false&sourceDictionaries=wiktionary&useCanonical=false&includeTags=false&api_key=br9ptmduqtp65ajnmis68uie4mo42twn92nr6j44z7s1ygznf`;
...
PetiteVue
    .createApp({
        ...
        reload() {
            ...
            // v-if
            new Promise(resolve => resolve()).then(() => that.definition = null);
